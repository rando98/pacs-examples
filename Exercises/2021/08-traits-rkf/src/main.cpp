...
main(int argc, char **argv)
{
  // Model problem.
  {
    ...
    const unsigned int n_max_steps = 1e2;

    RKF<RKFScheme::RK23_t, ProblemType::Scalar> solver; // (/* ??? */, f);

    RKFScheme::RK23_t  rk23_table;

    solver.set_table(rk23_table);
  ...
  //   std::cout << std::endl;
  //
    std::ofstream file("results_VdP_PROVA.out");
    file << solution;
  }
